...
app.post("/login", (req, res) => {
  logger.firstAuthentication(req.body).then(authValue => {
    if (authValue.status === 200) {
      const temp_id = security.temporary_id_generator(
        authValue.user._id.toString()
      );
...
app.post("/otp", (req, res) => {
  ...
  if (result.result) {
    logger.tokenIssuing(req.body.temp_id).then(tokenObject => {
  ...
  else res.status(500).json("Server malfunction");
});
